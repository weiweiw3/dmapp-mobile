...
(function (exports) {
    ...
    (function (exports, fb) {
        ...
        if (typeof(window) === 'undefined') {
            ...
            if (!Firebase) {
                throw new Error('Must include Firebase (http://cdn.firebase.com/v0/firebase.js) before firebase-util.js');
            }
        }
        ...
        util.toArray = function (vals, startFrom) {
            var newVals = util.map(vals, function (v, k) {
                return v;
            });
        ...
        util.isEmpty = function (vals) {
            return vals === undefined || vals === null ||
                ...
                (util.isObject(vals) && !util.contains(vals, function (v) {
                    return true;
                }));
        };
        ...
        util.find = function (vals, iterator, scope) {
            if (isArguments(vals)) {
                vals = Array.prototype.slice.call(vals, 0);
            }
            if (util.isArray(vals)) {
                for (var i = 0, len = vals.length; i < len; i++) {
                    if (iterator.call(scope, vals[i], i, vals) === true) {
                        return vals[i];
                    }
                }
            }
            ...
            if (a === b) {
                ...
                if (isA || isB) {
                    ...
                    return util.isEqual(aKeys, bKeys)
                        && !util.contains(a, function (v, k) {
                            return !util.isEqual(v, b[k])
                        });
                }
            }
            else {
    ...
    (function (exports, fb) {
        ...
        JoinedRecord.prototype = {
            ...
            onDisconnect: function () {
                throw new util.NotSupportedError('onDisconnect() not supported on JoinedRecord');
            },
            limit: function () {
                throw new util.NotSupportedError('limit not supported on JoinedRecord; try calling limit() on ref before passing into join');
            },
            endAt: function () {
                throw new util.NotSupportedError('endAt not supported on JoinedRecord; try calling endAt() on ref before passing into join');
            },
            startAt: function () {
                throw new util.NotSupportedError('startAt not supported on JoinedRecord; try calling startAt() on ref before passing into join');
            },
            transaction: function () {
                throw new util.NotSupportedError('transactions not supported on JoinedRecord');
            },
...
//               }
                ...
                if (this._isChildLoaded(rec)) {
                    ...
                    if (fromX > -1 && prevChild !== undefined) {
                        ...
                        if (prevChild !== null) {
                            toY = util.indexOf(this.sortedChildKeys, prevChild);
                            if (toY === -1) {
                                toY = this.sortedChildKeys.length
                            }
                        }
            ...
            _assertIsParent: function (fnName) {
                if (this.joinedParent) {
                    throw new Error(fnName + '() should only be invoked for parent records');
                }
            },
        ...
        function assertWritable(paths, onComplete) {
            var readOnlyNames = util.map(paths, function (p) {
                return p.isReadOnly() ? p.toString() : undefined
            });
        ...
        function extractValueForSetOps(value, primitivePath) {
            ...
            if (util.has(value, '.priority')) {
                out = util.filter(value, function (v, k) {
                    return k !== '.priority'
                });
            }
    ...
    (function (exports, fb) {
        ...
        JoinedSnapshot.prototype = {
            ...
            child: function () {
                return this.rec.child.apply(this.rec, util.toArray(arguments));
            },
            forEach: function (cb) {
            ...
            hasChildren: function () {
                return util.isObject(this.data) && !util.isEmpty(this.data)
            },
            ...
            numChildren: function () {
                return util.keys(this.data, function () {
                    return null
                }).length;
            },
            ref: function () {
    ...
    (function (fb) {
        ...
        KeyMapLoader.prototype = {
            ...
            _parseRawKeymap: function (km) {
                ...
                if (!util.isEmpty(dynamicPaths)) {
                    this.dynamicChildPaths = dynamicPaths;
                }
                if (!util.isEmpty(finalKeyMap)) {
                        ...
                        if (util.isObject(samplingSnap.val())) {
                            ...
                            samplingSnap.forEach(function (snap) {
                                ...
                                if (util.isObject(snap.val())) {
                                    ...
                                    util.each(snap.val(), function (v, k) {
                                        km[k] = k;
                                    });
                                    return false;
                                }
...
//            log('Path(%s) _parseRecord %s: %j', this.name(), snap.name(), out);
    ...
    (function (exports, fb) {
        ...
        function searchForParent(paths, childKey) {
            return util.find(paths, function (p) {
                return p.hasKey(childKey);
            }) || findSortPath(paths, intersections(paths));
        }
        ...
        function intersections(paths) {
            return util.filter(paths, function (p) {
                return p.isIntersection()
            });
        }
        ...
        function makeMasterName(paths) {
            var names = util.map(paths, function (p) {
                return p.ref().name();
            });
    ...
    (function (exports, fb) {
        ...
        function mergeIntersections(paths, valueParts) {
            var ikeys = util.map(paths.intersects, function (parts) {
                return parts[1];
            });
            var out = {};
        ...
        function pathParts(pendingPaths, sortPath, path) {
            ...
            if (path.isIntersection()) {
                pendingPaths.intersects.push(parts);
            }
            else {
                pendingPaths.unions.push(parts);
            }
